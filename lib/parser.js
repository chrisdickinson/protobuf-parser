...
function parser(fetch_import) {
  ...
  function parse_message_extensions() {
    ...
    if(node.stage === 0) {
      node.parent.extensions = []
      expect('extensions')
      return advance(true)
    }

    if(node.stage === 1) {
      node.parent.extensions[0] = parseInt(token.data, 10)
      return advance(true)
    }
    ...
    if(node.stage === 3) {
      node.parent.extensions[1] = token.data === 'max' ? Infinity : parseInt(token.data, 10)
      return advance(true)
    }
  ...
  function parse_field() {
    ...
    if(node.stage === 2) {
      if(!node.subtype) {
        node.subtype = stream.scope.lookup(node.children[0]).node
        if(!node.subtype) {
          return unexpected('expected a valid reference')
        }
      }
    ...
    if(node.stage === 5) {
      state.options = {}
      if(!('slot' in node)) {
        node.slot = stream.scope.lookup(node.children[node.children.length - 1]).node
      }
    ...
    if(node.stage === 1) {
      ...
      fetch_import(token.data, function(err, data) {
        ...
        tk.pipe(parse)
          ...
          .on('end', function() {
            stream.scope.root().merge(parse.scope.root())
            advance(true)
            stream.resume()
          })
  ...
  function parse_option() {
    ...
    if(node.stage === 4) {
      if(!('value' in node)) {
        node.value = node.parent.options[node.name] = stream.scope.lookup(node.children[0]).node
      }
